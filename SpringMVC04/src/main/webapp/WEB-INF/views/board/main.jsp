  ...
  <script type="text/javascript">
   ...
   var csrfTokenValue = "${_csrf.token}";
  	...
  	function makeView(data){ // data=[{ },{ },{ },,, ]
  		...
  		$.each(data, function(index,obj){ // obj={"idx":5,"title":"게시판"~~ }
  			listHtml+="<tr>";
  	  		...
  	  		listHtml+="<td>"+obj.indate.split(' ')[0]+"</td>"; // 공백을 기준으로 0번쨰 이후(시분초)를 자름
  	  		...
  	  		if("${mvo.memID}" == obj.memID){ // 쌍따옴표 붙여야 에러안남
  	  		listHtml+="<br/>"
  	...
  	function goInsert(){ 
  		...
  		var fData=$("#frm").serialize(); // 모든 데이터를 한 줄로 가져옴
  		// alert(fData);
  		$.ajax({
  			url : "board/new",
  			type : "post",
  		...
  		// $("#content").val("");
  		// $("#writer").val("");
  		$("#fclear").trigger("click"); // 글쓴다음 취소버튼을 추적해서 강제로 실행시킴
  	}
  	function goContent(idx){ 
  		...
  		$("#c"+idx).css("display","table-row"); // 선택한 게시글 보이게
...
<jsp:include page="../common/header.jsp"/>
  ...
  <div class="panel panel-default">
		 ...
		 <table class="table">
			...
			<tr>
				<td colspan="2" align="center">
					...
					<button type="button" class="btn btn-default btn-sm" onclick="goList()">리스트</button>
				</td>
			</tr>
    ...
    <div class="panel-footer">mvc04_유현준</div>
  </div>
</div>
